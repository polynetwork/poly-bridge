...
func (c *ExplorerController) GetAddressTxList() {
	...
	if addressTxListReq.ChainId == 0 {
		...
		res = db.Debug().Raw(`select a.hash, a.height, a.time, a.chain_id, b.from, b.to, b.amount, c.hash as token_hash, c.standard as token_standard, c.name as token_name, 1 as direct, m.precision, m.meta from src_transactions a left join src_transfers b on a.hash = b.tx_hash left join tokens c on b.asset = c.hash and b.chain_id = c.chain_id left JOIN token_basics m on c.token_basic_name = m.name and c.standard = 0 where b.from = ? and b.chain_id = ? 
		union select d.hash, d.height, d.time, d.chain_id, e.from, e.to, e.amount, f.hash as token_hash, f.standard as token_standard, f.name as token_name, 2 as direct, n.precision, n.meta from dst_transactions d left join dst_transfers e on d.hash = e.tx_hash left join tokens f on e.asset = f.hash and e.chain_id = f.chain_id left JOIN token_basics n on f.token_basic_name = n.name and f.standard = 0 where e.to = ? and e.chain_id = ? 
		order by height desc limit ?,?`,
	...
	if addressTxListReq.ChainId == 0 {
		res = db.Raw(`select sum(cnt) as counter from (select count(*) as cnt from src_transactions a left join src_transfers b on a.hash = b.tx_hash left join tokens c on b.asset = c.hash and b.chain_id = c.chain_id and c.standard = 0 where b.from = ? 
		union all select count(*) as cnt from dst_transactions d left join dst_transfers e on d.hash = e.tx_hash left join tokens f on e.asset = f.hash and f.standard = 0 and e.chain_id = f.chain_id where e.to = ?) as u`,
			addressTxListReq.Address, addressTxListReq.Address).
			Find(&counter)
	} else {
		res = db.Raw(`select sum(cnt) as counter from (select count(*) as cnt from src_transactions a left join src_transfers b on a.hash = b.tx_hash left join tokens c on b.asset = c.hash and b.chain_id = c.chain_id and c.standard = 0 where b.from = ? and b.chain_id = ? 
		union all select count(*) as cnt from dst_transactions d left join dst_transfers e on d.hash = e.tx_hash left join tokens f on e.asset = f.hash and f.standard = 0 and e.chain_id = f.chain_id where e.to = ? and e.chain_id = ?) as u`,
...
func (c *ExplorerController) GetCrossTx() {
	...
	err := db.Debug().Raw(`select hash from src_transactions where hash=?
