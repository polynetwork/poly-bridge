...
func (dao *BridgeDao) initEstimateProxy() {
	...
	proxyConfigs := dao.cfg.ChainListenConfig
...
func (dao *BridgeDao) initEstimateFeeMin() {
	...
	feeListenConfig := dao.cfg.FeeListenConfig
...
func NewBridgeDao(cfg *conf.Config, backup bool) *BridgeDao {
	swapDao := &BridgeDao{
		cfg:    cfg,
		backup: backup,
	}
	Logger := logger.Default
	if cfg.DBConfig.Debug == true {
		Logger = Logger.LogMode(logger.Info)
	}
	db, err := gorm.Open(mysql.Open(cfg.DBConfig.User+":"+cfg.DBConfig.Password+"@tcp("+cfg.DBConfig.URL+")/"+
		cfg.DBConfig.Scheme+"?charset=utf8"), &gorm.Config{Logger: Logger})
	if err != nil {
		panic(err)
	}
	swapDao.db = db
...
func (dao *BridgeDao) WrapperTransactionCheckFee(wrapperTransactions []*models.WrapperTransaction, srcTransactions []*models.SrcTransaction) error {
	logs.Info("check fee for poly wrapper Transaction")
	//get chain fee
	...
	for k, v := range wrapperTransactions {
		...
		fmt.Println("aaaa", feePay, feeMin)
