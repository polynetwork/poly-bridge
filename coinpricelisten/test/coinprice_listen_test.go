...
func TestListenFee(t *testing.T) {
	...
	chainFees := make([]coinpricelisten.ChainFee, 0)
	for _, cfg := range feeListenCfgs {
		chainFee := coinpricelisten.NewChainFee(cfg)
		chainFees = append(chainFees, chainFee)
	}
	feeListen := coinpricelisten.NewFeeListen(config.FeeUpdateSlot, chainFees, dao)
	feeListen.ListenFee()
}
