...
func setupApp() *cli.App {
	app := cli.NewApp()
	app.Usage = "poly-crossnative Service"
...
func startServer(ctx *cli.Context) {
	logs.SetLogger(logs.AdapterFile, `{"filename":"logs/poly_crossnative.log"}`)
...
func waitSignal() os.Signal {
	...
	go func() {
		for sig := range sc {
			logs.Info("cross nativetoken received signal:(%s).", sig.String())
			exit <- sig
			close(exit)
			break
		}
