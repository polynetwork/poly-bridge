...
func (this *ActivityStats) AirDropInfoStats() (err error) {
	timeNow := time.Now().Unix()
	if timeNow > this.cfg.AirDropEndTime+this.cfg.AirDropInfoInterval {
		return fmt.Errorf("END AirDropEndTime")
	}
...
func (this *ActivityStats) TokenPriceAvgStats() (err error) {
	...
	if timeNow > this.cfg.TokenPriceEndTime+this.cfg.TokenPriceAvgInterval {
		return fmt.Errorf("END TokenPriceEndTime")
	}
